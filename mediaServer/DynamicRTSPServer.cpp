...
static ServerMediaSession* createNewSMS(DynamicRTSPServer* rtsp, UsageEnvironment& env, char const* fileName, FILE* /*fid*/) {
  ...
  if (strcmp(extension, ".aac") == 0) {
    ...
    NEW_SMS("H.264 Video");
    OutPacketBuffer::increaseMaxSizeTo(100000); // allow for some possibly large H.264 frames
    sms->addSubsession(H264VideoFileServerMediaSubsession::createNew(env, fileName, reuseSource));
  } else if (strcmp(extension, ".265") == 0) {
    // Assumed to be a H.265 Video Elementary Stream file:
    NEW_SMS("H.265 Video");
    OutPacketBuffer::increaseMaxSizeTo(100000); // allow for some possibly large H.265 frames
...
//#define INTERLEAVE_ADUS 1
    ...
    OutPacketBuffer::increaseMaxSizeTo(300000); // allow for some possibly large VP8 or VP9 frames
